...
const Schedule = () => {
  ...
  useEffect(() => {
    if (allMatches.length > 0) {
      // Filter to show only upcoming matches in the schedule
      const upcomingScheduleMatches = allMatches.filter(match => 
        match.score.status === "Upcoming"
      );
      setMatches(upcomingScheduleMatches);

      // Group by date
      const grouped = allMatches.reduce<Record<string, Match[]>>((acc, match) => {
      ...
      if (selectedTab === "all") {
        ...
        setFilteredMatches(
          allMatches.filter((match) => match.competition.id === selectedTab)
        );
      }
      setLoading(false);
    }
  }, [allMatches, selectedTab]);

  // Update loading state
  useEffect(() => {
    setLoading(matchesLoading);
  }, [matchesLoading]);
