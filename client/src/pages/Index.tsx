...
const FAQSection = () => {
  ...
  const faqItems = [
    {
      id: 'watch-matches',
      question: t('howToWatch'),
      answer: t('howToWatchAnswer')
    },
    {
      id: 'free-service',
      question: t('isFree'),
      answer: t('isFreeAnswer')
    },
    {
      id: 'coverage',
      question: t('coverageQuestion'),
      answer: t('coverageAnswer')
    },
    {
      id: 'mobile-support',
      question: t('whatDevices'),
      answer: t('whatDevicesAnswer')
    },
    {
      id: 'upcoming-matches',
      question: t('howToFind'),
      answer: t('howToFindAnswer')
    },
    {
      id: 'streaming-issues',
      question: t('whyBuffering'),
      answer: t('whyBufferingAnswer')
    }
  ];
...
const Index = () => {
  ...
  // Update live and upcoming matches when matches data changes
  useEffect(() => {
    console.log("Index component - matches updated:", matches);
    console.log("Index component - matches length:", matches.length);
    ...
    if (matches && matches.length > 0) {
      ...
      console.log("Live matches found:", live);
      setLiveMatches(live);

      // Get upcoming matches - only check for "Upcoming" status
      console.log("All match statuses:", matches.map(m => ({ id: m.id, status: m.score.status })));
      const upcoming = matches.filter(match => 
        match.score.status === "Upcoming"
      );
      console.log("Upcoming matches found:", upcoming);
      setUpcomingMatches(upcoming);
    } else {
      console.log("No matches available or matches array is empty");
      setLiveMatches([]);
      setUpcomingMatches([]);
    }
  }, [matches]);
            ...
            <div className="p-4 lg:p-6">
              ...
              {hotMatches.length > 0 && (
                <section className="mb-6">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-lg font-semibold text-sport-text">{t('hotMatches')}</span>
                  </div>
                  <div className="space-y-3">
              ...
              {liveMatches.length > 0 && (
                <section className="mb-6">
                  <div className="flex items-center gap-2 mb-4">
                    <span className="text-sport-primary">🔴</span>
                    <span className="text-lg font-semibold text-sport-text">{t('liveMatches')}</span>
                  </div>
                  <div className="space-y-3">
              ...
              <section>
                <div className="flex items-center gap-2 mb-4">
                  <span className="text-sport-primary">📅</span>
                  <span className="text-lg font-semibold text-sport-text">{t('allMatches')}</span>
                </div>
                <div className="space-y-3">
                  {allMatches.length > 0 ? allMatches.map(match => (
                    ...
                    <div className="bg-sport-card border border-sport-border rounded-xl p-4 text-center">
                      <p className="text-sport-text-light text-sm">{t('noMatchesAvailable')}</p>
                    </div>
                  )}
                </div>
              </section>
              ...
              <section className="mt-8 md:mt-12">
                <div className="bg-white border border-gray-200 rounded-xl p-6 md:p-8 shadow-sm">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <h2 className="text-2xl md:text-3xl font-bold text-gray-900">{t('aboutUs')}</h2>
                  </div>
