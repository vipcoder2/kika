...
import { submitPollVote, getPollResults, checkUserVote } from "../services/matchesService";
...
const PollsVoting: React.FC<PollsVotingProps> = ({ match }) => {
  ...
  // Get user IP address (simplified - in production you might want a more robust solution)
  const getUserIP = async () => {
    try {
      ...
      return `fallback_${Date.now()}`; // Use timestamp as fallback for testing
    }
  };

  const fetchPollResults = async () => {
    try {
      const data = await getPollResults(match.id);
      setResults(data);
    } catch (error) {
  ...
  const checkUserVoteStatus = async () => {
    try {
      ...
      const data = await checkUserVote(match.id, ipAddress);
      setUserVote(data);
    } catch (error) {
  ...
  const handleVote = async (teamChoice: 'home' | 'away' | 'draw') => {
    ...
    try {
      ...
      console.log("Attempting to submit vote:", { matchId: match.id, teamChoice, ipAddress });

      // Submit vote directly - the Firebase function will handle duplicate checking
      await submitPollVote(match.id, teamChoice, ipAddress, userAgent);
      ...
      console.log("Poll results refreshed");
    } catch (error: any) {
      ...
      if (error.message && error.message.includes("already voted")) {
        ...
        // Update the user vote state to reflect they've already voted
        await checkUserVoteStatus();
      } else {
        setError(`${t('failedToSubmitVote')}: ${error.message || 'Unknown error'}`);
      }
    } finally {
      setIsVoting(false);
    }
    ...
    <section className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
      ...
      >
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-gray-800">{t('Match Poll')}</h2>
            <p className="text-xs text-gray-600">{t('Cast your prediction')}</p>
          </div>
        </div>
            ...
            <div className="space-y-3">
              <div className="text-center">
                <h3 className="text-sm font-medium text-gray-700 mb-3">
                  {t('whoWillWin')} {match.clubs.home.name} vs {match.clubs.away.name}
                </h3>
              </div>
              ...
              {isVoting && (
                <div className="text-center py-2">
                  <div className="inline-flex items-center space-x-2 text-blue-600">
                    ...
                    <span className="text-xs">{t('voting')}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="space-y-3">
              <div className="text-center">
                <div className="inline-flex items-center space-x-2 bg-green-50 border border-green-200 rounded-full px-3 py-1">
                  <span className="text-xs text-green-600 font-medium">✓ {t('voteCast')}</span>
                </div>
              </div>
              ...
              <div className="text-center text-xs text-gray-600 bg-gray-50 rounded-md py-2">
                <p>{t('totalVotes')}: {results.total} • {t('home')}: {results.home} • {t('draw')}: {results.draw} • {t('away')}: {results.away}</p>
              </div>
            </div>
          )}
        </div>
